...
import { addToCart } from "../../toolkit/reducers/CartActions";
import { page,cart } from "../../toolkit/reducers/UserAuth";
    ...
    if (IMAGE_URL) {
      dispatch(
        cart({
          ...
          owner_name: product.manu,
          amount: product.cost,
          crypto_price: product.nft,
        })
      );
    }
  ...
  return (
    <SafeAreaView>
      {loading ? ( 
        ...
        <SimpleAnimation delay={500} duration={500} fade>
    
          <ScrollView showsVerticalScrollIndicator={false} alwaysBounceVertical={true} contentContainerStyle={{paddingTop: vh(2),paddingBottom: vh(10),paddingLeft: vw(7),paddingRight: vw(7),}}>
            ...
            <View>
              ...
              { product.offers ? (<Text style={styles.desc}>Offers: {product.offers}.</Text>) : ('') }
           
              {/* Events for contacting for any queries */}
              <Text style={{ ...COMMONTEXT.primary, marginTop: vh(1) }}>For any Queries :{" "}</Text>

              <TouchableOpacity onPress={handleChat}>
                <Text style={[styles.text, { color: TEXTCOLOR.secondary }]}>Chat with Dealer via Whatsapp</Text>
              </TouchableOpacity>
            ...
            <View className={`flex flex-row ${product.nft ? `justify-between` : `justify-evenly`}`}>
              ...
              <TouchableOpacity className="shadow-lg shadow-black" style={styles.cartBtn} onPress={()=> addItemToCart(product)}>
                <Image source={icons.cart} style={{ width: 30, height: 30, alignSelf:'center'}}/>
              </TouchableOpacity>
